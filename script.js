...
// Función para renderizar el carrito (dibujar en html)
function agregarCarritoHTML() {
  ...
  totalAmount.innerHTML = "Total: " + totalPrice + " €";
}
...
function removeFromCart(productId) {
  const index = cart.findIndex(function(cartItem) {//carrItem es una  variable interna que usa la función, en este caso es un item 
    return cartItem.product.id === productId;
  });
  if (index !== -1) {//-1 es que no se encontró el producto
    cart.splice(index, 1);//splice es para borrar un elemento, index es el índice del elemento que quiero borrar y el 1 es el numero de elementos que queremos borrar
    agregarCarritoHTML();
  }
}
